# 라디오 버튼을 재클릭으로 해제하기 — 초기화 타이밍이라는 함정

## 🧩 처음 마주친 문제

신청 폼 페이지에서 라디오 버튼을 **재클릭하면 해제**되는 기능을 만들 일이 있었다. HTML 라디오는 기본적으로 한 번 선택되면 해제되지 않는 게 스펙이라, JS로 직접 구현해야 한다.
...
뭐가 달랐을까? 조회 페이지는 서버에서 이미 저장된 값을 불러와서 라디오에 **`checked` 속성이 미리 붙어있는 상태**다. 그 상태에서 첫 클릭이 헛발을 내는 것.
...
이게 서버가 렌더한 체크 상태를 잡아내서 `lastChecked`에 미리 넣어주니까, 첫 클릭에 바로 해제돼야 맞는데 왜 안 되지?
...
이 템플릿은 서버에서 즉시 치환되는 게 아니라 **프레임워크 JS가 나중에 비동기로 `checked` 상태를 설정**하는 경우가 많았다.
...
**내 초기 스캔은 허공에 대고 스캔**하고 있었던 거다. 라디오가 체크되기 **전**에 돌아서, `lastChecked`가 빈 채로 이벤트 핸들러가 실행됐다. 사용자 입장에선 첫 클릭이 그냥 날아간 셈.

---

## 💡 발상 전환 — 클릭 순간에 실제 상태를 보자

초기 스캔에 의존하는 한, 프레임워크 타이밍과 경쟁(race condition)에서 이길 수 없었다. **아예 스캔을 없애고, 클릭 순간에 현재 상태를 보는 방법**으로 바꾸기로 했다.

핵심 통찰은 이거다.

> `click` 이벤트는 브라우저가 상태를 **바꾼 후**에 발동한다.  
> `mousedown` 이벤트는 상태를 **바꾸기 전**에 발동한다.
...
### 1. "초기화 시점에 상태 스캔"은 프레임워크와 경쟁한다

내 JS가 깔끔하게 init해도, 프레임워크가 **나중에** DOM을 만지면 스캔 결과는 무의미해진다. 특히 Weven 같은 템플릿 치환 프레임워크, React/Vue 같은 선언적 라이브러리, 모두 **"초기 렌더 직후 상태"를 믿으면 위험**하다.

대안:
◦ 상태를 "지금 이 순간" 읽기 (event-driven)
◦ MutationObserver로 변화 감지
◦ `setTimeout(0)` 같은 지연 (불확실한 방법이라 비추)

### 2. 이벤트의 "전/후 타이밍"은 강력한 힌트다

`click`은 사용자 관점에선 한 번의 동작이지만, 브라우저 내부에선 여러 이벤트가 순서대로 발동된다.

```
mousedown → [state change] → mouseup → change → click
```

상태가 바뀌기 **전**의 값이 필요하면 `mousedown`이나 `keydown`을, **후**의 값이 필요하면 `click`이나 `change`를 쓰면 된다. 이 순서를 알면 "같은 동작인데 시점별로 다른 값을 봐야 할 때" 답이 나온다.

### 3. 전역 상태 맵보다 요소별 `.data()`가 깔끔하다

처음엔 `lastChecked = {}`라는 전역 맵에 그룹별 상태를 저장했다. 동작은 되지만:

◦ 라디오 그룹 이름을 키로 관리해야 함
◦ 디버깅할 때 맵 전체를 봐야 함
◦ DOM과 JS 상태가 따로 놀아서 일관성 깨지기 쉬움

mousedown 방식은 **각 요소에 `.data('wasChecked', ...)`로 상태를 붙임**. DOM 요소와 상태가 한 덩어리라 추적이 쉽고, 라디오 그룹이 몇 개든 자동 대응된다.

### 4. 엣지케이스도 생각하면 더 견고해진다

마우스뿐 아니라 **키보드로 Space**를 눌러 라디오를 조작할 수도 있다. 이 경우 `mousedown`은 안 뜬다. 완벽하게 하려면 `keydown`도 함께 다뤄야 한다.
...
처음엔 "코드는 잘 짠 것 같은데 왜 안 되지?"였다. 디버깅하면서 알게 된 건 **내 코드가 틀린 게 아니라, 내가 가정한 실행 순서가 틀렸다**는 사실이었다.

프레임워크 위에서 뭔가를 만들 땐:

> "내 코드가 돌아가는 시점에, DOM이 내가 기대한 상태인가?"

이 질문을 항상 던져야 한다. 초기화 시점에 DOM 상태를 캡처해서 쓰는 코드는 거의 언제나 잠재적인 타이밍 버그를 품고 있다. 가능하면 **"지금 이 순간" 상태를 보는 방식**으로 설계하면 좀 더 견고한 코드가 된다.
